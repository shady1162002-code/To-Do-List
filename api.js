...
const ProjectsAPI = {
    ...
    // Save all projects (bulk update)
    async saveAll(projects) {
        return await apiCall('/projects', 'POST', projects);
    },
    ...
    async save(project) {
        return await apiCall(`/projects/${project.id || ''}`, 'POST', project);
    },

    // Delete a project
    async delete(id) {
        return await apiCall(`/projects/${id}`, 'DELETE');
    }
};
...
async function checkServerHealth() {
    try {
        ...
        // Silently fail - this is expected when server is not running
